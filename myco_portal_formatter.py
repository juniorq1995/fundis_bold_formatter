...
def main():
    ...
    with alive_bar(len(inat_nums)) as bar:
        for idx, inat_num in enumerate(inat_nums):
            ...
            occurence_remarks = f"{notes}. {taste}. {odor}. {uv_reaction}. {koh_reaction}"
            ...
            for geo_result in geo_json['results']:
                ...
                for component in geo_result['address_components']:
                    if "administrative_area_level_1" in component['types'] and state == '':
                        state = component['long_name']
                    elif "administrative_area_level_2" in component['types'] and county == '':
                        county = component['long_name']  # county
            myco_portal.append([event_date, recorded_by, description, locality, latitude, longitude, county, state, scientific_name, catalog_number, habitat, substrate, occurence_remarks, associated_taxa])
            bar()
    myco_portal_df = pd.DataFrame(myco_portal, columns=[
        'eventDate',
        'recordedBy',
        'description',
        'locality',
    ...
    myco_portal_df.to_excel(os.path.join(myco_dir, 'MycoUpload.xlsx'), index=False, engine='xlsxwriter')
